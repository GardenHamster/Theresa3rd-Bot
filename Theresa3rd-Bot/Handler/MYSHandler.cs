    ...
    public class MYSHandler
    {
        ...
        public async Task subscribeMYSUserAsync(IMiraiHttpSession session, IGroupMessageEventArgs args, string message)
        {
            try
            {
                ...
                {
                    await session.SendGroupMessageWithAtAsync(args, "订阅失败，目标用户不存在");
                    return;
                }
                ...
                {
                    await session.SendGroupMessageWithAtAsync(args, $"已订阅了该用户~");
                    return;
                }
        ...
        public async Task cancleSubscribeMysUserAsync(IMiraiHttpSession session, IGroupMessageEventArgs args, string message)
        {
            try
            {
                ...
                {
                    await session.SendGroupMessageWithAtAsync(args, "并没有订阅这个用户哦~");
                    return;
                }
                ...
                await session.SendGroupMessageWithAtAsync(args, $"已为所有群退订了id为{userId}的米游社用户~");
                ConfigHelper.loadSubscribeTask();
            }
            catch (Exception ex)
            {
        ...
        private async Task<bool> CheckUserIdAsync(IMiraiHttpSession session, IGroupMessageEventArgs args, string value)
        {
            ...
            {
                await session.SendGroupMessageWithAtAsync(args, "用户id必须为数字");
                return false;
            }
            if (userId <= 0)
            {
                await session.SendGroupMessageWithAtAsync(args, "用户id无效");
                return false;
            }
            return true;
        }
        ...
        private async Task<bool> CheckSubscribeGroupAsync(IMiraiHttpSession session, IGroupMessageEventArgs args, string value)
        {
            ...
            if (string.IsNullOrWhiteSpace(value))
            {
                await session.SendGroupMessageWithAtAsync(args, "未指定目标群");
                return false;
            }
            if (int.TryParse(value, out typeId) == false)
            {
                await session.SendGroupMessageWithAtAsync(args, "目标必须为数字");
                return false;
            }
            if (Enum.IsDefined(typeof(SubscribeGroupType), typeId) == false)
            {
                await session.SendGroupMessageWithAtAsync(args, "目标不在范围内");
                return false;
            }
            return true;
        }
