    ...
    public class GroupMessagePlugin : BasePlugin
    {
        ...
        private async Task HandlemessageAsync(CqGroupMessagePostContext args)
        {
            try
            {
                ...
                {
                    GroupQuoteCommand quoteCommand = GetGroupQuoteCommand(args, message, instruction, prefix);
                ...
                if (isAt == false && isInstruct == false)//复读,分步操作,保存消息记录
                {
                    ...
                    Task task1 = RecordHelper.AddImageRecords(imgUrls, PlatformType.OneBot11, msgId, groupId, memberId);
                    List<string> plainMessages = args.Message.OfType<CqTextMsg>().Select(o => o.Text).ToList();
                    Task task2 = RecordHelper.AddMessageRecord(plainMessages, PlatformType.OneBot11, msgId, groupId, memberId);
                    return;
                }
                ...
                var command = GetGroupCommand(args, message, instruction, prefix);
