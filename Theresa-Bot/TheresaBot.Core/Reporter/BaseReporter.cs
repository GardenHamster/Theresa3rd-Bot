...
using TheresaBot.Core.Model.Error;

namespace TheresaBot.Core.Reporter
{
    public abstract class BaseReporter
    {
        ...
        public async Task SendError(Exception exception, string message = "")
        {
            try
            {
                if (BotConfig.ErrorPushGroups.Count == 0) return;
                ...
                foreach (var groupId in BotConfig.ErrorPushGroups)
                {
        ...
        public async Task SendErrorForce(Exception exception, string message)
        {
            try
            {
                var pushGroups = BotConfig.ErrorPushGroups;
                if (pushGroups.Count == 0) return;
                string sendMessage = $"{message}\r\n{exception.Message}\r\n{exception.StackTrace}";
                foreach (var groupId in pushGroups) await SendReport(groupId, sendMessage);
            }
            catch (Exception ex)
            {
                LogHelper.Error(ex, "错误日志发送失败");
